...
Import inside Blender and call the functions directly::
    ...
    import tanuki.mori.halo_maps.generators as gen
...
def generate_spawn_zones(
    ...
    print(f"[tanuki::halo_maps] Created {len(red_positions)} red and {len(blue_positions)} blue spawn points.")
...
def generate_base_volume(
    ...
    """Return a bpy script that creates the playable BSP volume geometry.
    ...
    * Bakes the resulting Geometry Nodes modifier to an actual mesh so the
    ...
    # Indent the DSL-generated block so it lives inside the function body of
    # the combined script.
    gn_block = indent(gn_script, "    ")

    bsp_name = repr(BSP_OBJECT_NAME)
...
if {bsp_name} not in bpy.data.objects:
    mesh = bpy.data.meshes.new({bsp_name})
    obj  = bpy.data.objects.new({bsp_name}, mesh)
    obj.location = (0.0, 0.0, 0.0)
    bsp_coll.objects.link(obj)
else:
    obj = bpy.data.objects[{bsp_name}]

bpy.context.view_layer.objects.active = obj

# ── Build the Geometry Node tree (Tanuki DSL compiled output) ─────────────
def _build_gn_tree(node_tree):
{gn_block}
...
print(f"[tanuki::halo_maps] bsp_world set to {{width}}×{{depth}}×{{height}} units.")
""".replace("{width}", str(width)).replace("{depth}", str(depth)).replace("{height}", str(height))
...
def generate_symmetry_layout(mode: str = "mirror") -> str:
    """Return a bpy script that places a symmetry reference Empty in the scene.
...
def generate_spawn_zones(
    ...
    """Return a bpy script that creates player spawn-point Empties.
...
red_positions  = {red_positions!r}
blue_positions = {blue_positions!r}
