...
def _spans(total: float, bed: float, overlap: float) -> list[tuple[float, float]]:
...
def tile_grid(width: float, height: float, bed_w: float, bed_h: float,
              *, overlap: float = 0.0) -> list[list[Rect]]:
    """Compute the rectangle grid (rows of columns) for the given bed."""
    xs = _spans(width, bed_w, overlap)
    ys = _spans(height, bed_h, overlap)
